...
//  WeatherAppNew
//
//  Created by Vladyslav Shepitko on 9/12/17.
...
class Cache<T : NSCoding > : NSObject {
    let name: String
    let cacheDirectiory:NSURL
    
    let queue = dispatch_queue_create("com.svapps.cacheQueue", DISPATCH_QUEUE_CONCURRENT)
    let fileManager = NSFileManager()
    let cache = NSCache()
    
    init(name:String, directory:NSURL?, fileProtection:String? = nil){
        self.name = name
        self.cache.name = name
        if let dir = directory {
            self.cacheDirectiory = dir
        }else{
            /*
